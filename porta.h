...
public:
  ...
  // Método clone (retorna uma cópia do objeto atual)
  ptr_Porta clone() const override {
    return new PortaNOT(*this);
  }

  // Retorna o nome da porta ("NT" para NOT)
  ...
  bool simular(const std::vector<bool3S>& in_port) override {
      // Verifica se o vetor de entradas tem exatamente uma entrada
      if (in_port.size() != 1) {
      setOutput(bool3S::UNDEF); // Define saída como indefinida se o tamanho for inválido
      return false;
      }

      // Realiza a operação NOT (negação)
      setOutput(!in_port[0]);
      return true;
  }
};
...
public:
  ...
  // DEMAIS FUNCOES DA PORTA
  // Construtor específico (recebe o número de entradas)
  PortaAND(int NI) : Porta(NI) {
      if (NI < 2) Nin_port = 0; // Número de entradas inválido
  }
  ...
  // Retorna o nome da porta ("AN" para AND)
  std::string getName() const override {
      return "AN";
  }
  ...
  bool simular(const std::vector<bool3S>& in_port) override {
      ...
      if (in_port.size() != getNumInputs()) {
          setOutput(bool3S::UNDEF); // Define saída como indefinida
          return false;
      }

      // Inicializa o resultado como TRUE e realiza a operação AND
      bool3S result = bool3S::TRUE;
      for (const auto& entrada : in_port) {
          result = result & entrada;
          if (result == bool3S::UNDEF) break; // Se indefinido, interrompe
      }

      setOutput(result); // Define o resultado como a saída
      return true;
  }
};
